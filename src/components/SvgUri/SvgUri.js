...
class SvgUri extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.name) {
      if (nextProps.name !== this.props.name) {
        this.fetchSVGData(nextProps.name);
      }
    }
  ...
  async fetchSVGData(name) {
    let responseXML = icons[name];
    ...
    Array.from(attributes).forEach(({ nodeName, nodeValue }) => {
      Object.assign(
        ...
        utils.transformStyle({
          ...
          strokeWidthProp: this.state.strokeWidth,
        })
      );
    });
    ...
    const componentAtts = Array.from(attributes)
      ...
      .reduce((acc, { nodeName, nodeValue }) => {
        ...
        if (
          ...
          nodeName === 'strokeWidth' &&
          nodeValue !== 'none'
        ) {
          strokeWidth = nodeValue;
        }
